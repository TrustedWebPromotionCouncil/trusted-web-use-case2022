...
class WoolletSocket {
    ...
    constructor() {
        this.socket = io(this.host, {path: this.path, transports : ['socketio', 'flashsocket', 'websocket', 'polling' ] });
    ...
    Events = {
        connect: () => { console.log('Socket connected (default event) ' + this.socket.id) },
        disconnect: () => { console.log('Socket disconnected') },
        connect_error: (e) => { console.log('Socket connect error', e) },
        error: () => { console.log('Socket error') },
        reconnect: () => { console.log('Socket reconnected') },
        reconnect_attempt: () => { console.log('Socket reconnect attempt') },
        reconnect_error: () => { console.log('Socket reconnect error') },
    ...
    messaging(i) {
        console.log(i);
        if (i.status == "success") {
            Dialog.notify('Message received', "Time: " + i.data.message.sent_time + "<br>From: " + i.data.message.connection_id + "<br><br>" + i.data.message.content);
        }
