...
impl TRNG {
    ...
    fn read_external(&self, extension: &Extension, size: &usize) -> Result<Vec<u8>, UNiDError> {
        ...
        if TRNG::MAX_BUFFER_LENGTH < *size {
            return Err(UNiDError { message: "Size is too large".to_string() })
        }
        ...
        unsafe {
            ...
            if result != 0 {
                return Err(UNiDError { message: "Failed to read".to_string() })
            }
    ...
    pub fn read(&self, size: &usize) -> Result<Vec<u8>, UNiDError> {
        ...
        let extension = match config.inner.lock() {
            ...
            _ => return Err(UNiDError { message: "Failed to load config".to_string() })
        };
