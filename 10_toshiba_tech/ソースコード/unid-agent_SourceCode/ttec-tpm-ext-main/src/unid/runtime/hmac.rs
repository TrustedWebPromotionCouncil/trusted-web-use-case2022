...
impl HmacSha256 {
    ...
    pub fn digest(secret: &[u8], message: &[u8]) -> Result<Vec<u8>, UNiDError> {
        let mut mac = match _HmacSha256::new_from_slice(secret) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to create HmacSha256".to_string()})
        };
    ...
    pub fn verify(secret: &[u8], message: &[u8], digest: &[u8]) -> Result<bool, UNiDError> {
        let computed = match HmacSha256::digest(secret, message) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to verify HmacSha256".to_string()})
        };
...
impl HmacSha512 {
    ...
    pub fn digest(secret: &[u8], message: &[u8]) -> Result<Vec<u8>, UNiDError> {
        let mut mac = match _HmacSha512::new_from_slice(secret) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to create HmacSha512".to_string()})
        };
    ...
    pub fn verify(secret: &[u8], message: &[u8], digest: &[u8]) -> Result<bool, UNiDError> {
        let computed = match HmacSha512::digest(secret, message) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to verify HmacSha512".to_string()})
        };
