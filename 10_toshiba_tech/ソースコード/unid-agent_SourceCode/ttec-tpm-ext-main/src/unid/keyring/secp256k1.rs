use crate::unid::sidetree::payload::PublicKeyPayload;
use crate::unid::{errors::UNiDError, runtime::base64_url::PaddingType};
use crate::unid::runtime;
...
impl Secp256k1 {
    ...
    pub fn new(context: &Secp256k1Context) -> Result<Self, UNiDError> {
        if context.secret.len() != Self::PRIVATE_KEY_SIZE {
            return Err(UNiDError{ message: "Invalid private key size".to_string() })
        }
        ...
        if context.public.len() == Self::COMPRESSED_PUBLIC_KEY_SIZE {
            let public = match Secp256k1::transform_uncompressed_public_key(&context.public) {
                Ok(v) => v,
                Err(_) => return Err(UNiDError{ message: "Failed to transform public key".to_string() })
            };
        ...
        Err(UNiDError{ message: "Invalid public key size".to_string() })
    }
    ...
    pub fn from_jwk(jwk: &KeyPairSecp256K1) -> Result<Self, UNiDError> {
        ...
        let x = match runtime::base64_url::Base64Url::decode_as_bytes(&jwk.x, &PaddingType::NoPadding) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to decode x".to_string()})
        };
        let y = match runtime::base64_url::Base64Url::decode_as_bytes(&jwk.y, &PaddingType::NoPadding) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to decode y".to_string()})
        };

        let public = [ &vec![ 0x04 ], &x[..], &y[..] ].concat();
        let private = match runtime::base64_url::Base64Url::decode_as_bytes(&d, &PaddingType::NoPadding) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to decode d".to_string()})
        };
    ...
    pub fn to_jwk(&self, included_private_key: bool) -> Result<KeyPairSecp256K1, UNiDError> {
        let validated = match self.validate_point() {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to validate point".to_string()})
        };

        if ! validated {
            return Err(UNiDError{ message: "Invalid point".to_string()})
        }

        let x = match self.get_point_x() {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to get point x".to_string()})
        };
        let y = match self.get_point_y() {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to get point y".to_string()})
        };
        ...
        if included_private_key {
            Ok(KeyPairSecp256K1 {
                ...
                d  : Some(runtime::base64_url::Base64Url::encode(&self.get_secret_key(), &PaddingType::NoPadding)),
                kid: None,
            })
        } else {
            Ok(KeyPairSecp256K1 {
    ...
    pub fn to_public_key(&self, key_id: &str, purpose: &Vec<&str>) -> Result<PublicKeyPayload, UNiDError> {
        let validated = match self.validate_point() {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to validate point".to_string()})
        };
        ...
        let jwk = match self.to_jwk(false) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to convert to JWK".to_string()})
        };
    ...
    pub fn get_point_x(&self) -> Result<Vec<u8>, UNiDError> {
        let public = self.get_public_key();

        if public.len() != Self::UNCOMPRESSED_PUBLIC_KEY_SIZE {
            return Err(UNiDError{ message: "Invalid public key size".to_string()})
        }
        if public[0] != 0x04 {
            return Err(UNiDError{ message: "Invalid public key format".to_string()})
        }
    ...
    pub fn get_point_y(&self) -> Result<Vec<u8>, UNiDError> {
        let public = self.get_public_key();
    ...
    pub fn validate_point(&self) -> Result<bool, UNiDError> {
        let x = match self.get_point_x() {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to decode x".to_string()})
        };
        ...
        let np = match IBig::from_str_radix("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F", 16) {
            Ok(v) => v,
            Err(_) => return Err(UNiDError{ message: "Failed to decode p".to_string()}),
        };
    ...
    pub fn transform_uncompressed_public_key(compressed: &[u8]) -> Result<Vec<u8>, UNiDError> {
...
pub mod tests {
    ...
    pub fn test_to_hex_key_pair() {
        ...
        assert_eq!(result.private, "c739805ab03da62ddbe03390acdf7615640aa6ed31b8f18243f04a572c528edb");
    ...
    pub fn test_to_jwk_with_private_key() {
        ...
        assert_eq!(result.d, Some("xzmAWrA9pi3b4DOQrN92FWQKpu0xuPGCQ_BKVyxSjts".to_string()));
    ...
    pub fn test_from_jwk_with_private_key() {
        ...
        assert_eq!(clone.get_secret_key(), vec![
