...
import { LOCAL_STORAGE_TRANSACTION_ID, LOCAL_STORAGE_VC_REQUEST_KEY } from "../../configs/constants";
...
export const Redirect: React.FC = () => {
  ...
  const { url, transactionID } = router.query;
  ...
  useEffect(() => {
    if (router && url) {
      (async () => {
        ...
        localStorage.setItem(LOCAL_STORAGE_TRANSACTION_ID, transactionID as string);
        ...
        try {
          ...
          router.push({
            pathname: "/result",
            query: { type: "loading", result: "false", errorMessage: "This URL is not suitable" },
          });
        }
